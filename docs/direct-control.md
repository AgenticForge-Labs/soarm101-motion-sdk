...
Each streamed sample is checked against calibrated/model joint limits, maximum command step, rate-aware joint speed and acceleration, following error, hardware faults, and motor-effort interlocks. Coarse workspace checks are applied to live streaming only when `teleop_workspace_checks=True`; they remain opt-in until the robot-specific table frame and tool geometry are calibrated. Normal point-to-point and trajectory motion is excluded while a joint stream is active.
