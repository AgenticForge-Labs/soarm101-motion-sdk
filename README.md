...
- **Go beyond servo angles.** The SDK includes forward and inverse kinematics plus Cartesian motion planning, providing a natural path from understanding individual joints to working in terms of tool position and linear movement in space. The Cartesian GUI uses 2 mm jogs by default, accepts short FIFO bursts of repeated jog clicks, shows requested-versus-achieved TCP diagnostics, and includes an interactive 3D view driven by the same native kinematic model used for planning.
