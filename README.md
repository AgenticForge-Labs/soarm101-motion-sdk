...
- **Calibration you can see and understand.** The guided mechanical-stop calibration visualizes each joint and the gripper as you move them through two complete sweeps. The inner ring tracks the outward sweep, the outer ring tracks the return, and each joint shows its measured range and completion state. You can finish early, cancel, and keep the previous calibration if a sweep fails.
